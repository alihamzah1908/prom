...
class TaskController extends Controller
{
    ...
    public function index_testing(){
        ...
        $tasks = Task::when(request()->id_type, function($tasks){
                    ...
                    })->when(request()->name_site_b, function($tasks){
                        $tasks = $tasks->whereHas('taskDetail', function($tasks){
                            $tasks = $tasks->whereHas('siteB', function($tasks){
                                return $tasks->where('name_site', 'like', '%'.request()->name_site_b.'%');
                            });
                        });
        ...
        $tasks_count = Task::when(request()->id_type, function($tasks_count){
                    ...
                    })->when(request()->name_site_a, function($tasks_count){
                        $tasks_count = $tasks_count->whereHas('siteA', function($tasks_count){
                            return $tasks_count->where('name_site', 'like', '%'.request()->name_site_a.'%');
                        });
                    })->when(request()->name_site_b, function($tasks_count){
                        $tasks_count = $tasks_count->whereHas('taskDetail', function($tasks_count){
                            $tasks_count = $tasks_count->whereHas('siteB', function($tasks_count){
                                return $tasks_count->where('name_site', 'like', '%'.request()->name_site_b.'%');
                            });
                        });
                    })->where('is_deleted', '=', '0')->latest()->count();
                    // dd($tasks);
        $link_task_1 = null;
        if(request()->link_task_from){
            $find_task_1 = Task::where('task_uid', request()->link_task_from)->first();
            if($find_task_1){
                $link_task_1 = $find_task_1;
            }
        }
        $link_task_2 = null;
        if(request()->link_task_to){
            $find_task_2 = Task::where('task_uid', request()->link_task_to)->first();
            if($find_task_2){
                $link_task_2 = $find_task_2;
            }
        }
        if(request()->link_task_from == request()->link_task_to){
            $link_task_1 = null;
            $link_task_2 = null;
        }
        $types  = TaskType::all();
        ...
        return view('task.index_testing', compact('tasks', 'types', 'regions', 'statuses', 'priorities', 'roots', 'tasks_count', 'link_task_1', 'link_task_2'));
    }
    public function index_schedule(){
       // 25-07-22
        //  $task_sche = TaskSchedule::when(request()->id_type, function ($task_sche) {
        //     $task_sche = $task_sche->where('id_task_type', request()->id_type);
        // })->when(request()->id_region, function ($task_sche) {
        //     $task_sche = $task_sche->where('id_region', request()->id_region);
        // })->when(request()->id_status, function ($task_sche) {
        //     $task_sche = $task_sche->where('id_status', request()->id_status);
        // })->when(request()->created_at_when, function ($task_sche) {
        //     $task_sche = $task_sche->where('created_at', '>=', date('Y-m-d 00:00:00', strtotime(request()->created_at_when)));
        // })->when(request()->created_at_until, function ($task_sche) {
        //     $task_sche = $task_sche->where('created_at', '<=', date('Y-m-d 23:59:59', strtotime(request()->created_at_until)));
        // })->when(request()->id_task, function ($task_sche) {
        //     $task_sche = $task_sche->where('task_uid', 'like', '%' . request()->id_task . '%');
        // })->when(request()->subject, function ($task_sche) {
        //     $task_sche = $task_sche->where('subject', 'like', '%' . request()->subject . '%');
        // })->when(request()->id_priority, function ($tasks) {
        //     $task_sche = $task_sche->whereHas('taskDetail', function ($task_sche) {
        //         return $task_sche->where('id_priority', request()->id_priority);
        //     });
        // })->when(request()->id_root, function ($task_sche) {
        //     $task_sche = $task_sche->whereHas('taskDetail', function ($task_sche) {
        //         return $task_sche->where('id_root_caused', request()->id_root);
        //     });
        // })->when(request()->completed_at_when, function ($task_sche) {
        //     $task_sche = $task_sche->whereHas('taskDetail', function ($task_sche) {
        //         return $task_sche->where('request_complete_time', '>=', date('Y-m-d 00:00:00', strtotime(request()->completed_at_when)));
        //     });
        // })->when(request()->completed_at_until, function ($task_sche) {
        //     $task_sche = $task_sche->whereHas('taskDetail', function ($task_sche) {
        //         return $task_sche->where('request_complete_time', '<=', date('Y-m-d 23:59:59', strtotime(request()->completed_at_until)));
        //     });
        // })->when(request()->technician, function ($task_sche) {
        //     $task_sche = $task_sche->whereHas('technician', function ($task_sche) {
        //         return $task_sche->where('name_technician', 'like', '%' . request()->technician . '%');
        //     });
        // })->where('is_deleted', '=', '0')->latest()->paginate(20);
        // 25-07-22 end
        
        $task_sche = TaskSchedule::where('is_deleted', '=', '0')->latest()->paginate(20);
        
        
        $data['title'] = 'Task';
        $types = \App\Model\TaskType::get();
        $regions  = Region::all();
        $statuses = \App\Model\Status::where('task_type_id', '=', 2)->get();
        $technician = \App\Model\Technician::get();
        $priorities = Priority::all();
        $roots = RootCaused::all();
        // $task_sche = TaskSchedule::paginate(20);
        return view('task_schedule.index', 
            compact('task_sche', 'types', 'regions', 'statuses', 'priorities', 'roots', 'technician'));
    }
    ...
    public function delete_task(Request $r,$id_task){
        ...
        return redirect()->route('task.testing');
    ...
    public function getDataReport(Request $r){
       ...
       // if($r->is_link_task) $data = $data->whereIn('id_status', [1,2,3,4,5]); 
        ...
        if($r->created_by) $data = $data->where('tb_task.created_by', $r->created_by);
        ...
        if($completion_from && $completion_to) $data = $data->whereBetween('tb_task.time_complete', ["$completion_from 00:00:00", "$completion_to 23:59:59"]);
    ...
    public function getData(Request $r){
     ...
     public function getData2(Request $r){
        $user = Auth::user();
        $technician = Technician::where('user_id', $user->id)->first();
    ...
    public function getDataSchedule(Request $r){
       ...
       // if($r->is_link_task) $data = $data->whereIn('id_status', [1,2,3,4,5]); 
        ...
        // add filter created_at
        if(request()->created_at_when || request()->created_at_until) {
            if(request()->created_at_when && request()->created_at_until){
                // $data = $data->whereBetween('created_at', ["request()->created_at_when 00:00:00", " 23:59:59"]);
                $data = $data->whereBetween('created_at', [date('Y-m-d', strtotime(request()->created_at_when))." 00:00:00", date('Y-m-d', strtotime(request()->created_at_until))." 23:59:59"]);
            }else{
                if(request()->created_at_when){
                    $data = $data->whereDate('created_at', date('Y-m-d', strtotime(request()->created_at_when)));
                }elseif(request()->created_at_until){
                    $data = $data->whereDate('created_at', date('Y-m-d', strtotime(request()->created_at_until)));
                }
    ...
    public function new_task(Request $r){
        ...
        return redirect()->route('task.testing')->with('message', "Task Created Successfully!")->with('alert-class', 'alert-success')->with('data', $task);
    }
    ...
    public function new_link_task(Request $r){
        $task_1 = Task::where('task_uid', $r->id_task_1)->first();
        $task_2 = Task::where('task_uid', $r->id_task_2)->first();
        
        $link = new TaskLink;
        $link->id_task_1 = $task_1->id_task;
        $link->id_task_2 = $task_2->id_task;
        $link->id_technician_1 = $task_1->technician->id_technician;
        $link->id_technician_2 = $task_2->technician->id_technician;
        $link->save();
        
        \Session::flash('message', "Task Linked Successfully!");
        \Session::flash('alert-class', 'alert-success');
        return redirect()->route('task.testing')->with('message', "Task Link Successfully!")->with('alert-class', 'alert-success');
    }
