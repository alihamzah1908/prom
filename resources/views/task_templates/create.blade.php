...
<form method="POST" action="/setup/template-form/{{$id_template}}/new_task_templates_value?id={{request()->id}}" class="form-new-task-template_b">
@csrf
<div class="card martop-1">
    <div class="card-header">
        <div class="row">
            <div class="col-md-7">
                <h3 class="card-title">{{ __('setup-templateform-index-create2.template_language') }}</h3>
            </div>
            
            <div class="col-md-2 mt-1 text-right">
                <b class="">{{ __('setup-templateform-index-create2.copy-from_language') }} </b>
            </div>
            <div class="col-md-3">
                <select class="form-control" onchange="document.location.href='?id={{request()->id}}&template_name={{request()->template_name}}&from='+this.value">
                    <option value="" selected disabled>- {{ __('setup-templateform-index-create2.copy-template_language') }} -</option>
                    @foreach(\App\Templates::where('id_task_type', $id_template)->get() as $t)
                        <option value="{{$t->id}}" {{request()->from == $t->id ? 'selected':''}}>{{$t->template_name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
                ...
                <div class="card p-3">
                    <div class="row">
                        <div class="form-group col-md-6">
                            <label>{{ __('setup-templateform-index-create2.template-name_language') }}</label>
                            <input class="form-control" placeholder="{{ __('setup-templateform-index-create2.template-name2_language') }}" name="template_name" required value="{{$temp->template_name}}">
                        </div>
                        <div class="form-group col-md-6">
                            <label>{{ __('setup-templateform-index-create2.comments_language') }}</label>
                            <input class="form-control" placeholder="{{ __('setup-templateform-index-create2.comments2_language') }}" name="template_comments" value="{{$temp->template_comments}}">
                        </div>
                    </div>
                </div>
                <div class="card">
            ...
            <div class="col-md-2 bg-grays card display-nones">
                <div class="card-header">
                    <div class="card-title">
                        {{ __('setup-templateform-index-create2.elements_language') }}
                    </div>
                </div>
    ...
    <div class="card-footer">
        ...
        <a class="btn btn-md btn-default" href="/setup/template-form/{{$id_template}}">{{ __('setup-templateform-index-create2.back_language') }}</a>
        <button type="submit" class="btn btn-md btn-success">{{ __('setup-templateform-index-create2.save-template_language') }}</button>
    </div>
</div>
</form>
        ...
        <div class="modal-content">
            <form method="POST" action="#" class="form-update-element">
                <div class="modal-header">
                    <h3 class="modal-title">{{ __('setup-templateform-index-create2.field-set_language') }}</h3>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label>{{ __('setup-templateform-index-create2.field-id_language') }}</label>
                        <input class="form-control" readonly name="field_id">
                    </div>
                    <div class="form-group">
                        <label>{{ __('setup-templateform-index-create2.field-type_language') }}</label>
                        <input class="form-control" readonly name="field_type">
                    </div>
                    <div class="form-group">
                        <label>{{ __('setup-templateform-index-create2.field-name_language') }}</label>
                        <input class="form-control" placeholder="{{ __('setup-templateform-index-create2.field-name2_language') }}" name="field_name">
                    </div>
                    <div class="form-group">
                        <label>{{ __('setup-templateform-index-create2.default-value_language') }}</label>
                        <input class="form-control" placeholder="{{ __('setup-templateform-index-create2.default-value2_language') }}" name="field_default_value">
                    </div>
                </div>
                <div class="modal-footer">
                    <a href="#" class="btn btn-info btn-sm text-light" style="font-weight:bold; font-size:15px" data-dismiss="modal">{{ __('setup-templateform-index-create2.close_language') }}</a>
                    <button class="btn btn-success btn-updatae-element">{{ __('setup-templateform-index-create2.save_language') }}</button>
                </div>
            </form>
        </div>
    </div>
