...
<div class="row mb-2">
    <div class="col-sm-6">
        <h3 class="m-0 text-dark"><strong>Checklist</strong></h3>
    </div>
</div>
@endsection
@section('content')
    ...
    <div class="card-header">
        ...
        <div class="card-tools">
            <button class="btn btn-sm btn-success btn-new-data">
                <i class="fa fa-plus"></i> New Checklist
            </button>
        </div>
    </div>
    <div class="card-body">
...
<div class="modal fade" tabindex="-1" id="modal_new_data" role="dialog" aria-labelledby="warning">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="POST" action="new_checklist" class="form-new-data">
            @csrf
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input class="form-control" name="checklist_name" placeholder="Checklist" required autofocus>
                    </div>
                    <div class="form-group">
                        <label>Description</label>
                        <textarea class="form-control" name="checklist_desc" placeholder="Description" required autofocus rows="2"></textarea>
                    </div>
                </div>
...
$(document).ready(function () {
    ...
    $(document).on('click', '.checklist_name', function(e){
        ...
        var ini = $(this);
            tr = ini.parents('tr'),
            id = tr.find('.id_checklist').html(),
            mdl = $('#modal_update_data');
        
        if($.isNumeric(id)) {
            mdl.modal('show');
            form = $('.form-update-data');
            form.attr('action', 'update_checklist/'+id);
            ...
            .done(function (result) {
                ...
                if(result.data[0]){
                    var data = result.data[0],
                        ...
                        form.find("textarea[name=checklist_desc]").val(data.checklist_desc);
                }else{
    ...
    $(document).on('submit', '.form-new-data', function(e){
        ...
        var post_data = {
            ...
            checklist_name: ini.find("input[name=checklist_name]").val(),
            checklist_desc: ini.find("textarea[name=checklist_desc]").val(),
        };
        ...
        function postData(post_data) {
            ...
            .done(function (result) {
                ...
                if (result.status) {
                    ...
                    ini.find("textarea[name=checklist_desc]").val(null);
                    ini.find("input[name=checklist_name]").val(null);
...
function loadThisTable(param){
    ...
    th = '<th width="5%">ID</th>' +
         '<th class="text-dark cursor-default">Checklist</th>' +
         '<th>Desc</th>'+
         '<th>CreatedBy</th>'+
         '<th>LastUpdateBy</th>';
    columns = [
                {"data": "id_checklist", className: "id_checklist"},
                {"data": "checklist_name", className: "checklist_name text-link"},
                {"data": "checklist_desc"},
