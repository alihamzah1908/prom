        ...
        <div class="tab-content">
            ...
            <div class="tab-pane" id="approval" style="padding: 1rem;">
                ...
                <div class="row">
                    ...
                    <div class="modal fade" id="modalApprovalCheckout">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form method="POST" action="/site-permit/site-entry-approval-checkout/{{$data->id_site_entry}}">
                                @csrf
                                    <div class="modal-header">
                                        <h4 class="modal-title">Site Entry Approval</h4>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    ...
                                    <div class="modal-body">
                                        <div class="form-group text-center">
                                            <img src="/images/approve.png">
                                            <h5 class="mt-3 mb-4">
                                               Site Entry will be <span class="approval_status_checkout"></span>!
                                               <br>
                                               Please leave note: 
                                            </h5>
                                            <input hidden value="" name="approval_status_checkout">
                                            <textarea class="form-control" name="note" required autofocus></textarea>
                                        </div>
                                        <button type="submit" class="btn btn-primary float-right">Submit</button>
                                    </div>
                            ...
                            <div class="card-body">
                                <table>
                                    <thead>
                                        ...
                                        <tr>
                                            <th>STATUS</th>
                                            <th>: {{$data->approver_1_checkout}}</th>
                                        </tr>
                                        <tr>
                                            <th>NOTE</th>
                                            <th>: {{$data->approver_1_notecheckout}}</th>
                                        </tr>
                                    </thead>
                                </table>
                            </div>
                           @if($data->approver_2_status == "APPROVED" && !$data->approver_2_checkout)
                            @if(Auth::user()->id == $data->approver_1)
                            <div class="card-footer">
                                <button class="btn btn-sm btn-success float-right text-light btn-site-approval-checkout" data-status="APPROVED">APPROVE</button>
                                <button class="btn btn-sm btn-danger float-right text-light btn-site-approval-checkout mr-2" data-status="REJECTED">REJECT</button>
                            </div>
                            @endif
                            @endif
                        </div>
                            ...
                            <div class="card-body">
                                <table>
                                    <thead>
                                        ...
                                        <tr>
                                            <th>STATUS</th>
                                            <th>: {{$data->approver_2_checkout}}</th>
                                        </tr>
                                        <tr>
                                            <th>NOTE</th>
                                            <th>: {{$data->approver_2_notecheckout}}</th>
                                        </tr>
                                    </thead>
                                </table>
                            </div>
                            @if($data->approver_1_checkout == "APPROVED" && !$data->approver_2_checkout)
                            @if(Auth::user()->id == $data->approver_2)
                            <div class="card-footer">
                                <button class="btn btn-sm btn-success float-right text-light btn-site-approval-checkout" data-status="APPROVED">APPROVE</button>
                                <button class="btn btn-sm btn-danger float-right text-light btn-site-approval-checkout mr-2" data-status="REJECTED">REJECT</button>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '.btn-site-approval', function(e){
         ...
         $(document).on('click', '.btn-site-approval-checkout', function(e){
            e.preventDefault();
            var ini = $(this),
                status = ini.data('status');
                console.log(status)
                
            $('.approval_status_checkout').html(status);
            $('input[name=approval_status_checkout]').val(status);
            $('#modalApprovalCheckout').modal('show');
