...
<script>
    ...
    function initDashboard(param){
        $.get('/api/getSiteEntryDashboardData', param, function(response){
            var data = response;
            
            by_status = data.by_status;
            putChart($('#card_by_status'), 'by_status', 'Status', by_status.colors, by_status.labels, by_status.dataset, 'DONUT', 'SITE_ENTRY_STATUS'); 
            ...
            putChart($('#card_by_region'), 'by_region', 'by_region', '#007bff', by_region.labels, by_region.dataset, 'BAR', 'SITE_ENTRY_REGION');
            
            by_site = data.by_site;
            putChart($('#card_by_site'), 'by_site', 'by_site', '#007bff', by_site.labels, by_site.dataset, 'BAR', 'SITE_ENTRY_SITE');
        });
    }
